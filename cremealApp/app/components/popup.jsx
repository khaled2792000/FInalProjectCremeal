...
const PopupMenu = ({ meal, user })  => {
    ...
    const AddToFavorite = () => {
        ...
        Add_To_Favorite(meal.id, user.jwtToken)
          .then((result) => {
            console.log(result)
      ...
      const RemoveFromFavorite = () => {
        console.log(meal.id)
        Remove_From_Favorite(meal.id, user.jwtToken)
          .then((result) => {
            console.log(result)
            if (result.status === 200) {
                setLiked(false);
            ...
            <Modal transparent visible={visible}>
                <SafeAreaView style={{ flex: 1 }} onTouchStart={TouchStart}>
                    ...
                    >
                        ...
                        >
                            <Text>Favorite</Text>
                            <MaterialIcons name={liked ? "favorite" : "favorite-outline"} size={32} color={liked ? "black" : "black"} style={{ marginLeft: 10 }} />
                        </TouchableOpacity>
                        <TouchableOpacity
                            onPress={() => {
                                inviteOnPress()
